...
begin
  ...
  // Gestion de cas où aucun maillon n'est dans la liste
  if _current.List <> nil then begin
    // Récupération du chaînage du maillon en tête de liste
    _current.List^.getNearDynPix(_lprev,_lnext);
    // Il n'y a pas de chaînon avant puisqu'on travaille sur l'ancre
    // Affectation du chainage du nouveau maillon
    // Prev = nil (tête de chaîne) Next = maillon actuellement en tête de liste = List
    _pt^.setNearDynPix(nil,_current.List);
    // Mise à jour de maillon existant
    // Prev = pt, Next = next
    _current.List^.setNearDynPix(_pt, _lnext);
  end;

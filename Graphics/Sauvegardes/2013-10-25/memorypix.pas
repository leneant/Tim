...
uses
  Classes, SysUtils, ExtCtrls, Graphics, ProgressWindows, GraphType;
...
begin
  ...
  //datasizePTR := Image.Picture.Bitmap.RawImage.DataSize;
  //size := @datasizePTR;
  // Pointer on picture data
   ...
   for i := 0 to yend - 1 do begin
    for j := 0 to xend - 1 do
      begin
          ...
          // Blue
          //inc(ScanLine);
          //position.Byte := PtrUInt(ScanLine+1 + 4*(i*xend+j));
          //position.Bit := 32;
          //Image.Picture.Bitmap.RawImage.WriteChannels(position,R,G,B,L);
          inc(ScanLine);
          ScanLine^ := B;
          // Green
          inc(ScanLine);
          ...
          inc(ScanLine);
          ScanLine^ := L;
          //Image.Picture.Bitmap.Canvas.Pixels[j,i] := RGBToColor(trunc(R*L/255),trunc(G*L/255),trunc(B*L/255));
      end;

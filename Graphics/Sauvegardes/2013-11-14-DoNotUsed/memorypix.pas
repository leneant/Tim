...
uses
  Classes, SysUtils, ExtCtrls, Graphics, ProgressWindows, GraphType, TSL, TabChainee;
...
type
  ...
  TDynTblPix = packed record
    _mem : array [1..TTBL_Max] of TPixel;
    _gmem : TTBL_Chaine;
  end;

  PTDynTblPix = ^TDynTblPix;

  TAncrePixels = packed record
    nbPixels : integer;  // Nombre de pixels dans la liste (pour la barre de progression des traitements)
    List : LongInt;      // La liste elle même (index du premier maillon en mémoire
    _current : LongInt;  // Le maillon courant de la liste
  end;
  ...
  TListPixels = class(TObject)
    private
      ...
      procedure _getCoord(var _current : TAncrePixels; var x,y : integer);
    ...
    public
      ...
      constructor Create(var _mem : TDynTblPix);
      destructor Destroy;
      procedure Clear;
      ...
      procedure getImageFromTImage (var Image : TImage);
      procedure drawTeinteIntoTImage(var Teinte : TAncrePixels;var Image : TImage; _draw : boolean; sat, lum : integer; waitingbox, buffered : boolean; wboxpercent : integer);
  end;
...
var _R,_G,_B : Byte;
begin
  if _current._current <> 0 then begin
    _memory^._mem[_current._current]._pixel._R := R;
    _memory^._mem[_current._current]._pixel._G := G;
    _memory^._mem[_current._current]._pixel._B := B;
    _memory^._mem[_current._current]._pixel._L := L;
    _R := R*L div 255;
    _G := G*L div 255;
    _B := B*L div 255;
    _memory^._mem[_current._current]._satcoef := TSL_getSATcoef(_R,_G,_B);
    _memory^._mem[_current._current]._x := x;
    _memory^._mem[_current._current]._y := y;
  end;
end;

procedure TListPixels._add(var _current : TAncrePixels; var x,y : integer; var R,G,B,L : Byte);
var _pt, __next, __prev, _lprev, _lnext : LongInt ;
  _R,_G,_B : Byte;
begin
  // Creation du nouveau maillon
  _pt := TTBL_new(_memory^._gmem);
  if _pt = 0 then exit;
  _memory^._mem[_pt]._pixel._R := R;
  _memory^._mem[_pt]._pixel._G := G;
  _memory^._mem[_pt]._pixel._B := B;
  _memory^._mem[_pt]._pixel._L := L;
  _R := R*L div 255;
  _G := G*L div 255;
  _B := B*L div 255;
  _memory^._mem[_pt]._satcoef := TSL_getSATcoef(_R,_G,_B);
  _memory^._mem[_pt]._x := x;
  _memory^._mem[_pt]._y := y;
  _memory^._mem[_pt]._next := _current.List;
...
procedure TListPixels._getCoord(var _current : TAncrePixels; var x,y : integer);
begin
  if _current._current <> 0 then begin
     x := _memory^._mem[_current._current]._x;
     y := _memory^._mem[_current._current]._y;
  end;
end;

procedure TListPixels._getPixel(var _current : TAncrePixels; var x,y : integer; var R,G,B,L : Byte);
begin
  if (_current._current > 0) and (_current._current < _memory^._gmem._ancre._deb) then begin
    R := _memory^._mem[_current._current]._pixel._R;
    G := _memory^._mem[_current._current]._pixel._G;
    B := _memory^._mem[_current._current]._pixel._B;
    L := _memory^._mem[_current._current]._pixel._L;
    x := _memory^._mem[_current._current]._x;
    y := _memory^._mem[_current._current]._y;
  end;
end;
...
begin
  if (_current._current > 0) and (_current._current < _memory^._gmem._ancre._deb) then begin
    coefs := _memory^._mem[_current._current]._satcoef;
  end;
end;
...
begin
  if _current._current <> 0 then
    _current._current := _memory^._gmem._list[_current._current]._next;
end;

function TListPixels._isEnd(var _current : TAncrePixels) : boolean;
begin
  if _current._current <> 0 then
    _isEnd := _current._current >= _memory^._gmem._ancre._free
  else _isEnd := true;
end;
...
// Public methods.
constructor TListPixels.Create(var _mem : TDynTblPix);
begin
  _memory := @_mem;
  TTBL_Init(_mem._gmem);
  _create(IOO);
  with IOORGB do
  begin
      _SR := 0;
...
procedure TListPixels.drawTeinteIntoTImage(var Teinte : TAncrePixels; var Image : TImage; _draw : boolean; sat, lum : integer; waitingbox, buffered : boolean; wboxpercent : integer);
{
...
var i : LongInt ; // pour la barre de progression
  ...
  _R,_G,_B,_L : Byte;
  _RC,_GC,_BC : real;
  _RL, csat, clum,_sr,_sg,_sb : real;
  _coefs : TCoefSatDSat;
  _color : TColor;
  Bmp : TBitmap;
  _current : LongInt;
begin
  if buffered then begin
    ...
    if _draw then begin
      ...
      csat := sat / 500 * 255;
      clum := lum / 500 * 255;
      while _current <> 0 do begin
          Teinte._current := _current;
          _get(Teinte,_x,_y,_R,_G,_B,_L,_coefs);
          _RL := _L/255.0;
          _sr := _coefs.r*csat;
          _sg := _coefs.g*csat;
          _sb := _coefs.b*csat;
          ...
          _GC := _G*_RL+_sg+clum;
          _BC := _B*_RL+_sb+clum;
          if _RC < 0 then _RC := 0 else if _RC > 255 then _RC := 255;
          if _GC < 0 then _GC := 0 else if _GC > 255 then _GC := 255;
          if _BC < 0 then _BC := 0 else if _BC > 255 then _BC := 255;
          Bmp.Canvas.Pixels[_x,_y] := RGBToColor(round(_RC),round(_GC),round(_BC));
          // Chaînage
          self._next(Teinte);
          _current := Teinte._current;
          inc (i);
          if (i mod 1500 = 0) and waitingbox then ProgressWindow.setProgressInc(round(i/Teinte.nbPixels*wboxpercent));
      end;
    end else begin
     // boucle pour effacer les pixels
      ...
      while _current <> 0 do begin
          Teinte._current := _current;
          _getCoord(Teinte, _x,_y);
          Bmp.Canvas.Pixels[_x,_y] := _color;
          // Chaînage
          self._next(Teinte);
          _current := Teinte._current;
          inc (i);
    ...
    if _draw then begin
      ...
      while _current <> 0 do begin
          ...
          if _GC < 0 then _GC := 0 else if _GC > 255 then _GC := 255;
          if _BC < 0 then _BC := 0 else if _BC > 255 then _BC := 255;
          Image.Picture.Bitmap.Canvas.Pixels[_x,_y] := RGBToColor(round(_RC),round(_GC),round(_BC));
          // Chaînage
          self._next(Teinte);
          _current := Teinte._current;
          inc (i);
     ...
     // boucle pour effacer les pixels
      ...
      while _current <> 0 do begin
          _getCoord(Teinte, _x,_y);
          Image.Picture.Bitmap.Canvas.Pixels[_x,_y] := _color;
          // Chaînage
          self._next(Teinte);
          _current := Teinte._current;
          inc (i);
...
begin
  ...
  for i := 0 to self._height - 1 do begin
      ...
      if (y mod 5 = 0) then ProgressWindow.setProgressInc(y/self._height*70);
  end;
end;
